...
import '../devoluciones/devoluciones_screen.dart';  // ✅ FIX: doble slash eliminado
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<_MenuItem> _getMenuItems(String rol) {
    final items = <_MenuItem>[
      _MenuItem(icon: Icons.point_of_sale_rounded,
          label: 'POS / Ventas',      roles: ['cajero']),
      _MenuItem(icon: Icons.inventory_2_rounded,
          label: 'Inventario',         roles: ['admin', 'supervisor', 'cajero']),
      _MenuItem(icon: Icons.account_balance_wallet_rounded,
          label: 'Caja',              roles: ['cajero']),
      _MenuItem(icon: Icons.bar_chart_rounded,
          label: 'Contabilidad',       roles: ['admin', 'supervisor', 'cajero']),
      _MenuItem(icon: Icons.assessment_rounded,
          label: 'Reportes',           roles: ['admin', 'supervisor', 'cajero']),
      _MenuItem(icon: Icons.people_rounded,
          label: 'Empleados',          roles: ['admin']),
      _MenuItem(icon: Icons.store_rounded,
          label: 'Tiendas',            roles: ['admin']),
      _MenuItem(icon: Icons.people_alt_rounded,
          label: 'Clientes',           roles: ['admin', 'supervisor', 'cajero']),
      _MenuItem(icon: Icons.local_shipping_rounded,
          label: 'Proveedores',        roles: ['admin', 'supervisor']),
      _MenuItem(icon: Icons.shopping_cart_rounded,
          label: 'Compras',            roles: ['admin', 'supervisor']),
      _MenuItem(icon: Icons.assignment_return_rounded,
          label: 'Devoluciones',       roles: ['admin', 'supervisor', 'cajero']),
    ];
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ FIX: postFrameCallback — no mutar estado directamente en build
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text(
                            ...
                            style: GoogleFonts.poppins(
                              color: _rolColor(auth.rol), fontSize: 9,
                              fontWeight: FontWeight.w600)),
                        ),
                      ],
                    ),
          ...
          // ── Contenido principal ──────────────────────────
          // ✅ FIX: IndexedStack — mantiene estado de todas las pantallas
          Expanded(
            child: IndexedStack(
