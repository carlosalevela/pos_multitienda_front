...
class ReporteTabla extends StatelessWidget {
              ...
              builder: (context, constraints) {
                ...
                return SingleChildScrollView(
                  ...
                  child: ConstrainedBox(
                    constraints: BoxConstraints(minWidth: constraints.maxWidth > minWidth
                        ? constraints.maxWidth
                        : minWidth),
                    child: DataTable(
                      showCheckboxColumn: false,
                      headingRowColor: WidgetStateProperty.all(kDark),
          ...
          cells: [
            ...
            DataCell(
              Row(
                children: [
                  Icon(
                    iconMetodo(v['metodo_pago'] ?? ''),
                    size: 13,
                    color: colorMetodo(v['metodo_pago'] ?? ''),
                  ),
                  const SizedBox(width: 5),
                  Flexible(child: Text(labelMetodo(v['metodo_pago'] ?? ''))),
                ],
              ),
            ),
            DataCell(
          ...
          cells: [
            ...
            DataCell(
              Row(
                children: [
                  Icon(
                    iconMetodo(a['metodo_pago'] ?? ''),
                    size: 13,
                    color: colorMetodo(a['metodo_pago'] ?? ''),
                  ),
                  const SizedBox(width: 5),
                  Flexible(child: Text(labelMetodo(a['metodo_pago'] ?? ''))),
                ],
              ),
            ),
            DataCell(
  ...
  List<DataRow> _devolucionesRows() => rep.devoluciones.map((d) {
        final hora = (d['created_at']?.toString() ?? '').length >= 19
            ...
            : '';
        final total = double.tryParse(d['total_devuelto']?.toString() ?? '0') ?? 0;
        ...
        return DataRow(
          color: WidgetStateProperty.all(kDevColor.withOpacity(0.05)),
          cells: [
            DataCell(
              Text(
                factura,
            ...
            DataCell(
              Row(
                children: [
                  ...
                  const SizedBox(width: 5),
                  Flexible(child: Text(labelMetodo(metodo))),
                ],
              ),
            ),
            DataCell(
